/************************************************************************
 *
 * anomaly_macros_rtl.h : $Revision: 5445 $
 *
...
**
**  ADSP-BF561 - from rev 0.0-0.4 (fixed 0.5)
**
...
#define WA_05000127 \
  ((defined(__SILICON_REVISION__) && defined(__ADSPBF561__)) && \
   (__SILICON_REVISION__ == 0xffff || __SILICON_REVISION__ <= 0x4))
...
/* 05-00-0227 - Scratchpad Memory Bank Reads May Return Incorrect Data 
**
** BF531/2/3 - 0.0-0.4 (fixed 0.5)
** BF538     - 0.1     (fixed 0.2)
** BF539     - 0.0-0.1 (fixed 0.2)
...
/* 05-00-0257 - Interrupt/Exception During Short Hardware Loop May Cause
**              Bad Instruction Fetches
**
** Unpredictable behavior can result when hardware loops shorter than 4
** instructions in length are interrupted at the end of the loop due to an
** interrupt or exception.  In this situation, the processor's loop buffers,
** which are used to reduce the instruction fetch latency, operate incorrectly,
** resulting in the wrong instructions being fetched as the loop exits.
**
** BF531/2/3 - 0.0-0.4 (fixed 0.5)
** BF534/6/7 - 0.0-0.2 (fixed 0.3)
** BF538/9   - 0.1-0.2 (fixed 0.3)
** BF561     - 0.2-0.3 (fixed 0.5)
**
** Used by VDK
*/
#define WA_05000257 \
