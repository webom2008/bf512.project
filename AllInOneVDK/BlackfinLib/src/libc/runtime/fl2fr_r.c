 ...
 *************************************************************************/
    ...
    Function: float_to_fr16_rnd - convert 32-bit floating point
                                  value to signed fract value with
                                  rounding
    ...
    Implementation:
        ...
        Extract mantissa and shift to produce a s1.15 result.
...
#pragma file_attr("libFunc=float_to_fr16_rnd")
#pragma file_attr("libFunc=__float_to_fr16_rnd")
...
extern fract 
float_to_fr16_rnd(float parm1)
{
    /* the bits of the s1.15 result. */
    int16_t result;
    ...
    */
    ...
    else if (exponent > 110U)
    {
        ...
        int32_t adj_exponent = ((int32_t)exponent - 127) + 7;
        full_result = (uint64_t)umantissa;
        full_result |= 0x100000000ULL;

        /* shift the mantissa into the two parts of the s24.39 result */
        ...
        result = (int16_t)__builtin_ashiftll((int64_t)full_result, -24);
    ...
    return rbits(result) + round_amount;
}
