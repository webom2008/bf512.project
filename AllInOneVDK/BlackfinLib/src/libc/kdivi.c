 ...
 *************************************************************************/
    ...
    Function: kdivi - divide an int by an int to give an accum result

    Synopsis:

        accum kdivi (int dividend, int divisor);

    Description:
 
        This function divides an integer argument by an 
        integer argument to produce an accum result. The result
    ...
    Error Conditions:
        ...
        If the result overflows the accum size, the value is
        saturated to accum.
...
#pragma file_attr("libFunc=kdivi")
#pragma file_attr("libFunc=_kdivi")
      ...
      int _divisor) 
{
    ...
    if ((dividend != 0) && (divisor != 0))
    {
        ...
        */
        ...
        uint_uk_t result_bits_minus_1 = (uint_uk_t)ACCUM_FBIT
                                      + (uint_uk_t)ACCUM_IBIT - 1U;
        ...
        */
        ...
        {
            /* and convert to accum. */
            result = kbits((int_k_t)i_result);
            if (negate)
            {
                result = -result;
            }
