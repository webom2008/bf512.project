 ...
 *************************************************************************/
    ...
    Function: urdivi - divide an unsigned int by an unsigned int to give an 
                       unsigned fract result

    Synopsis:

        unsigned fract urdivi (unsigned int dividend, unsigned int divisor);

    Description:
 
        This function divides an unsigned integer argument by an unsigned
        int argument to produce unsigned fract result. The result
    ...
    Algorithm:
        ...
        number of bits in an unsigned int and an unsigned fract is at most
        64.
    ...
    Example:
        ...
        result = urdivi (dividend,divisor);
...
#pragma file_attr("libFunc=urdivi")
#pragma file_attr("libFunc=_urdivi")
       ...
       unsigned int _divisor) 
{
    ...
    */
    ...
    ull_dividend <<= (uint64_t)UFRACT_FBIT;
    ...
    */
    ...
    if (ull_quotient > (uint64_t)bitsur(UFRACT_MAX))
    {
        result = UFRACT_MAX;
    }
    else
    {
        ...
        result = urbits((uint_ur_t)ull_quotient);
    }
    return result;
}
