/*
** Write "data" to L1 Instruction memory.
...
** write to Blackfin's L1 Instruction memory, which ordinarily
** would raise an exception. The function copies n bytes from
** datap to instrp, and returns instrp. It has some restrictions
...
.file_attr libFunc=__memcpy_l1;
.file_attr FuncName=__memcpy_l1;
...
__memcpy_l1:
   ...
   [--SP] = (R7:4);
	...
	P0 = R1;	// For reading the data to be written
	P1 = R0;	// For returning at end of function
...
// 0xFF600000 - 0xFF603FFF  Code SRAM (16K)
   ...
   // otherwise fall thru to check core A at .check_generic
#else
...
/* __ADSPBF535__ core only */
	...
	// Zero ITEST_COMMAND (in case it has crud in it and
...
.ls:	// Compute the command from the physical address
	R2 = R0 & R5;		// Bits 9:3 as Index and Double-word index
	R4 = R0 & R6;		// Bits 13:12 as sub-bank
	R4 <<= 4;
	R2 = R2 | R4;
	R4 = R0 & R7;		// Bits 11:10 as Way
	R4 <<= 16;
	R2 = R2 | R4;
	BITSET(R2, 1);		// Select Write command
	BITSET(R2, 2);		// Select Data array
	BITTGL(R2, 27);		// Way bits are inverted!
	BITTGL(R2, 26);

	// Read the instructions from the data buffer
	R1 = [P0++];
	R3 = [P0++];

	// Put them into the Write registers
	[I2] = R1;
	[I3] = R3;
	CSYNC;
...
.le:	R0 += 8;
   ...
   CLI R7;        // disable interrupts before ITEST_COMMAND till after CSYNC
	...
	[I0] = R3;
   // suppress assembler warning for 05-00-0312 for CSYNC because interrupts
   // are disabled.
   ...
   CLI R7;        // disable interrupts before DTEST_COMMAND till after CSYNC
	// Zero DTEST_COMMAND (in case it contains a command
	// that will overwrite something)
	[I0] = R3;
	R2 >>= 3;		// Divide loop count by 8
   // suppress assembler warning for 05-00-0312 for CSYNC and LC0 load 
   ...
   STI R7;        // reenable interrupts
	...
	R5 = 0x47F8 (Z);  // here for anomalies 05-00-0244 and 05-00-0283
	R1.L = 0x0006;
	R1.H = 0x0100;

	// Configure the SRAM/Cache to be in SRAM mode.
	[I1] = R3;
    ...
    R2.L = LO(0xFFA04000);  // L1 instruction bank B
    R2.H = HI(0xFFA04000);
    R4=R2-R0;
#if __SILICON_REVISION__ != 0xffff
    P4=0;
...
.ils:	
   ...
   // Suppress the assembler warning as the following loads should
   // always be valid as it's the data pointer supplied by the user.
   .MESSAGE/SUPPRESS 5508 FOR 3 LINES;
#endif
   R2 = [P0++];   // Read the data to be copied
   R4 = [P0++];
   [I2] = R2;     // Store in the Write registers
	[I3] = R4;
#if defined(__WORKAROUND_SYNC_LOOP_ANOM_312)
	CLI R2;
#endif
	...
	R2 = R0 & R5;		// Bit 14, Index and Double-word Index
	R4 = R0 & R6;		// Sub-bank
	R4 <<= 4;
	R2 = R2 | R4;
	R4 = R0 & R7;		// Way
	R4 <<= 15;
	R2 = R2 | R4;
	R2 = R2 | R1;		// Select Data Array, Instr Access, Write
#if !defined(__ADSPBF561__) && !defined(__ADSPBF50x__) && \
    !defined(__ADSPBF59x__)
            // DTEST_COMMAND:23 is always 0 for BF561 and BF59x parts instr mem
            // and reserved for BF50x parts
   CC = R3 <= R0 (IU);  // Select block
   R4 = CC;
   R4 <<= 23;
   R2 = R2 | R4;
#endif
   ...
   STI R3;
#endif
   R3 = [I3];        // read DTEST_DATA1
.redo2:              // repeat write command till we are sure it has worked
   BITSET(R2, 1);    // Ensure write bit is set
   ...
   CSYNC;
   STI R4;           // reenable interrupts
   R4 = [I2];        // read DTEST_DATA0
   CC = R1 == R4;    // check if data read is what we wanted to write
   IF !CC JUMP .redo2;
...
#endif
	R4 = [I3];        // Read DTEST_DATA1, ea5517 and ea5508 might be suppressed
	CC = R3 == R4;    // check if write succeeded
	IF !CC JUMP .redo2;
	R1 = P5;
	R3 = P3;
#else
...
.reading_via_itest_command:
	...
	CLI R5;
	[--SP] = R5;
	.MESSAGE/SUPPRESS 5515;	// suppress assembler warning for 05-00-0312
...
.dls:
   ...
   NOP;
#endif
	[I2] = R2;		// Put into Write registers
	[I3] = R4;
	CSYNC;

	// Construct the address command from the instrp address
	R2 = R0 & R5;		// Index and Double-word index : ADR[9:3]
	R4 = R0 & R6;		// sub-bank : ADR[13:12]
	R4 <<= 4;
	R2 = R2 | R4;
	R4 = R0 & R7;		// Way (and no inverting on LP cores) : ADR[11:10]
	R4 <<= 16;
	R2 = R2 | R4;
	BITSET(R2, 2);		// Data array
.redo:
	BITSET(R2, 1);		// Write command

	// Issue command - interrupts already disabled
#if WA_05000123
	[I0] = R2;
	CSYNC;
	BITCLR(R2, 1);		// Read command
...
#if WA_05000123 && WA_05000428
   // If working around WA_05000123 and WA_05000428, then the
   // workaround for WA_05000123 above means that we need a
   // workaround for WA_05000428 (Speculative read from L2 by
   // Core B may cause a write to L2 to fail).
	NOP;
	NOP;
#elif WA_05000245
   // 050000245: False Hardware Error from an Access in the Shadow of 
   //            a Conditional Branch
   // Suppress the assembler warning as the following stack load should
   // always be valid.
   .MESSAGE/SUPPRESS 5508 FOR 2 LINES;
#endif
   R0 = [SP++];            // Re-enable interrupts
   STI R0;
   .MESSAGE/RESTORE 5515;  // restore assembler warning for 05-00-0312
