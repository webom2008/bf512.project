 ...
 *************************************************************************/
    ...
    Function: roundur - round an unsigned fract value to a given
                         precision

    Synopsis:

        unsigned fract roundur (unsigned fract parm,
                                int num_fract_bits);

    Description:
 
        This function rounds a given unsigned fract parameter 
    ...
    Error Conditions:
        ...
        If the input precision is greater than or equal to UFRACT_FBIT, 
...
#pragma file_attr("libFunc=roundur")
#pragma file_attr("libFunc=_roundur")
...
roundur(unsigned fract _f,
        int _num_fract_bits) 
{
    const unsigned fract f = _f;
    const int num_fract_bits = _num_fract_bits;

    /* the result after rounding */
    unsigned fract result;

    if (expected_false((num_fract_bits >= UFRACT_FBIT) ? (int)1 : (int)0))
    {
    ...
    else
    {
        ...
        /* the unsigned fract bit-pattern interpreted as an
        ** unsigned integer.
        */
        uint_ur_t f_ui = bitsur(f);
