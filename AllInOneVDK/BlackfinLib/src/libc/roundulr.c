 ...
 *************************************************************************/
    ...
    Function: roundulr - round an unsigned long fract value to a given
                         precision

    Synopsis:

        unsigned long fract roundulr (unsigned long fract parm,
                                      int num_fract_bits);

    Description:
 
        This function rounds a given unsigned long fract parameter 
        ...
        of the RND_MOD bit. The rounding may saturate, so that 0xffffffff
        is a valid output.

    Error Conditions:

        If the input precision is greater than or equal to ULFRACT_FBIT, 
...
#pragma file_attr("libFunc=roundulr")
#pragma file_attr("libFunc=_roundulr")
...
roundulr(unsigned long fract _f,
         int _num_fract_bits) 
{
    ...
    if (expected_false((num_fract_bits >= ULFRACT_FBIT) ? (int)1 : (int)0))
    {
    ...
    else
    {
        ...
        /* the unsigned long fract bit-pattern interpreted as an
        ** unsigned integer.
        */
        uint_ulr_t f_ui = bitsulr(f);
