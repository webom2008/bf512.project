...
#pragma file_attr ("prefersMemNum=70")
    ...
    Function: strtofxuk - parse a string representing an unsigned accum
    ...
    Description:
        ...
        characters as an unsigned accum value. If successful, this value
        ...
        a valid representation of an unsigned accum, endptr is set to
        point to the beginning of the original string.

    Error Conditions:

        If the string represents a value outside of the range of an unsigned
        accum, a saturated result is returned and errno is set to
        ERANGE.
    ...
    Example:
        ...
        #include <stdfix.h>
        char *my_string = " 0.5 is an unsigned accum value";
        char **tail;
        unsigned accum result;

        result = strtofxuk (my_string,tail);
...
unsigned accum 
strtofxuk(const char * __restrict _nptr,
          char ** __restrict _endptr)
{
    ...
    */
    ...
    return ukbits((uint_uk_t)result_i);
}
