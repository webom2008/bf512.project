...
static char   	*pEzKitTitle 	= 	"ADSP-BF561 EZ-KIT Lite";
static char 	*pFlashDesc		=	"STMicro. M29W640DT";
...
ERROR_CODE GetCodes(int *pnManCode, int *pnDevCode, unsigned long ulAddr)
{
	...
	ReadFlash( ulFlashStartAddr + 0x0400, (unsigned short *)pnManCode );
	*pnManCode &= 0x00FF;
	 	
	// now we can read the codes
	ReadFlash( ulFlashStartAddr + 0x0402, (unsigned short *)pnDevCode );
	*pnDevCode &= 0xFFFF;
			
	 // M29W640DT
	 if( (*pnManCode == 0x20) && (*pnDevCode == FLASH_DEVCODE_ST_M29W640DT ) )
	 {
	  	pFlashDesc =	"STMicro. M29W640DT";
	 }
	 // M29W640FT
	 else if( (*pnManCode == 0x20) && (*pnDevCode == FLASH_DEVCODE_ST_M29W640FT ) )
	 {
		pFlashDesc =	"STMicro. M29W640FT";
	 }
	 else
...
ERROR_CODE GetSectorNumber( unsigned long ulAddr, int *pnSector )
{
	...
	long lBootStart = 0x7F0000;
	long lBootEnd = 0x800000;
...
ERROR_CODE GetSectorStartEnd( unsigned long *ulStartOff, unsigned long *ulEndOff, int nSector )
{
	...
	unsigned long  ulBlkSize0 	= 0x10000;	// Block size 0
	unsigned long  ulBlkSize1	= 0x2000;	// Block size 1
