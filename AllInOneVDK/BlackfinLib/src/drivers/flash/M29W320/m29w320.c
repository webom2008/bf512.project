...
#ifdef __ADSPBF518__
static char *pBoardDesc = 		"ADSP-BF518F EZ-Board";	/* description of supported platform */
...
ERROR_CODE m29w320_Open(void)
{
	...
	if ( (FLASH_MANUFACTURER_ST != g_ManId) ||
			( (FLASH_DEVCODE_ST_M29W320EB != g_DevId) && (FLASH_DEVCODE_ST_M29W320DB != g_DevId) ) )
	{
		return UNKNOWN_COMMAND;
	}

	/* now that we know what we're connected to, update some variables */
	if ( FLASH_DEVCODE_ST_M29W320EB == g_DevId)
	{
		/* 32 Mbit (4 MB) device */
		gNumSectors = 71;
...
/* if we are protecting a sector we need to know its boundaries */
    ...
    for (i = 0; i < uiCount; i++, ulCurrentAddress++, pusCurrentData++)
    {
...
#endif
		{
			...
			WriteFlash( ulFlashStartAddress + 0x0554, 0x55 );
