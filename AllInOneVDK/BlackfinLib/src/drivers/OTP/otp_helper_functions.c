...
#ifdef _USING_OTP_BASIC_

/* this will not use any system services or device driver interfaces */
...
/* if you are using system services or device driver interfaces */
...
#ifdef _USING_OTP_BASIC_
	Result = otp_basic_Read(page, flags, page_content);
#else
    ADI_DEV_1D_BUFFER OTPBuff1D;		/* buffer pointer */
...
#ifdef _USING_OTP_BASIC_
	Result = otp_basic_Write(page, ADI_OTP_CHECK_FOR_PREV_WRITE | flags, page_content);
#else
    ADI_DEV_1D_BUFFER OTPBuff1D;		/* buffer pointer */
    ADI_OTP_INFO OTP_Params;
    
    OTP_Params.page = page;
	OTP_Params.flags = ADI_OTP_CHECK_FOR_PREV_WRITE | flags;
