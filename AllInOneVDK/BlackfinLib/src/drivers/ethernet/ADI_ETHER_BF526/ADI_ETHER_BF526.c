...
Description: This is Ethernet device driver for ADI BF526 EZ-kit that
...
#include <drivers/ethernet/ADI_ETHER_BF526.h>       // BF526 ether driver includes
#include <ccblkfn.h>
#include <cdefbf526.h>
#include <time.h>
...
typedef struct adi_ether_bf526_data {
    void                    *CriticalData;
    ...
    ADI_ETHER_BF526_TRACE_DATA  Trc;        // trace support data
...
static ADI_ETHER_BF526_DATA EtherDev;               // control data pointer
...
static DumpTraceBuffer(void)
{
    if (EtherDev.MaxTraceEntries>0) {
        ADI_ETHER_BF526_DATA *dev = &EtherDev;
        int i,j;
        ADI_ETHER_BF526_TRACE_ENTRY *te;

        te = (dev->Trc.NoOfEntries<=dev->MaxTraceEntries?dev->Trc.BaseEntry:dev->Trc.OldestEntry);

        for (i=0;i<dev->Trc.NoOfEntries;i++) {
            te = (ADI_ETHER_BF526_TRACE_ENTRY *)(((char *)te) + dev->Trc.EntryLnth);
            switch (te->Dirn) {
            case 'T':
            case 'R':
...
static void QueueFrames(ADI_ETHER_BF526_DATA *dev, FRAME_QUEUE *q);
...
ADI_DEV_PDD_ENTRY_POINT ADI_ETHER_BF526_Entrypoint = {
    adi_pdd_Open,
    adi_pdd_Close,
    adi_pdd_Read,
    adi_pdd_Write,
...
#ifdef ADI_ETHER_BF526_DEBUG
#define CheckBuffers() ADI_ETHER_BF526_CheckBuffers()
void ADI_ETHER_BF526_CheckBuffers(void);
#else
...
XBUFFER *ADI_ETHER_BF526_FreeBuf;
int ADI_ETHER_BF526_NoOfBuffers;
int ADI_ETHER_BF526_NoOfFreeBuffers;
int ADI_ETHER_BF526_NoPosted;
void *ADI_ETHER_BF526_PostedBuffer;
int ADI_ETHER_BF526_OutstandingPosts;
...
void ADI_ETHER_BF526_CheckBuffers()
{
    int nr,nt,nf,nx;
    XBUFFER *xb;
    ...
    nf = ADI_ETHER_BF526_NoOfFreeBuffers;
    if (nt+nr+nf+ADI_ETHER_BF526_NoPosted < ADI_ETHER_BF526_NoOfBuffers-1) breakpoint(4);
}
...
//      set up the phy
//
static SetPhy(ADI_ETHER_BF526_DATA *dev)
{
    int count;
    u16 phydat,phydat1;
    clock_t ndtime;
    ...
    /* Set the power down flag if power down is enabled */
...
*
*   Description:    Opens the BF526 EMAC for use
*
*********************************************************************/
...
)
{
    ...
    if (!dev->Open) {
        ...
        memset(dev,0,sizeof(ADI_ETHER_BF526_DATA));
...
{
    ADI_ETHER_BF526_DATA *dev = (ADI_ETHER_BF526_DATA *)PDDHandle;
...
//      Queue new frames
//
static void QueueNewFrames(ADI_ETHER_BF526_DATA *dev, FRAME_QUEUE *q,ADI_ETHER_BUFFER *bfs)
{
...
//
static int StartMac(ADI_ETHER_BF526_DATA *dev)
{
    int res = 1; // failed
    u32 opmode;
    u16 sysctl;
    ...
    // The BF526 EZ-kit are shipped with POST that will set up core and system speed.
    ...
    if (res == 0) {
        ...
        if (res == 0) {
            ...
            *pEMAC_MMC_CTL = 0x0d;
            ...
            // Select RMII, BF526 kit only support RMII
            opmode = LCTRE | RMII;  //
            if (dev->StripPads) {
...

{
    ...
    char *EndXmit;
    ADI_ETHER_BF526_TRACE_INFO *trcinfo;
    ...
    switch (Command) {
        ...
        case ADI_ETHER_CMD_SET_PHY_POWER_DOWN:
            ...
            if (dev->Started) {
                ...
                // Wait for 1ms for PLL to stabilize 
                period = ((clock_t)nClocksPerSecond)/1000;
                ndtime = clock()+period;
                while (clock()<ndtime);
            }
            break;          
        ...
        case ADI_ETHER_CMD_BF526_NO_RCVE_LNTH:
            i = (int)(pArg);
            dev->NoRcveLnth = (i!=0);
            break;
        case ADI_ETHER_CMD_BF526_STRIP_PAD:
            i = (int)(pArg);
            dev->StripPads = (i!=0);
            break;
        case ADI_ETHER_CMD_BF526_CLKIN:
            i = (int)(pArg);
            dev->CLKIN = i;
            break;
        case ADI_ETHER_CMD_BF526_USE_IVG:
            ivgs = (ADI_ETHER_IVG_MAPPING *)(pArg);
            dev->EtherIntIVG = ivgs->ErrIVG;
            dev->RXIVG = ivgs->RxIVG;
            dev->TXIVG = ivgs->TxIVG;
            break;
        case ADI_ETHER_CMD_BF526_USE_DMA:
            dmas = (ADI_ETHER_DMA_MAPPING *)(pArg);
            dev->Rx.Channel = dmas->RxChannel;
            dev->Tx.Channel = dmas->TxChannel;
            break;
        case ADI_ETHER_CMD_BF526_SET_PHY_ADDR:
            i = (int)(pArg);
        ...
        case ADI_ETHER_CMD_GEN_CHKSUMS:
            dev->GenChksums = true;
            break;
        case ADI_ETHER_CMD_BF526_SET_TRACE:
            trcinfo = (ADI_ETHER_BF526_TRACE_INFO *)(pArg);
            dev->Trc.BaseEntry = (ADI_ETHER_BF526_TRACE_ENTRY *)trcinfo->Mem;
            dev->Trc.EntryLnth = (sizeof(ADI_ETHER_BF526_TRACE_ENTRY)+trcinfo->MaxBytes+3)&(~0x3); // round up to multiple of 4
            ...
            if (dev->Trc.NoOfEntries <=0) {
                ...
                dev->Trc.EndOfData = (ADI_ETHER_BF526_TRACE_ENTRY *)(((char *)dev->Trc.BaseEntry) + dev->Trc.EntryLnth*dev->Trc.NoOfEntries);
                dev->MaxTraceEntries = dev->Trc.NoOfEntries;
                dev->Trc.OldestEntry = dev->Trc.BaseEntry;
                dev->Trc.NoOfEntries = 0;
                dev->TraceMaxBytes = dev->Trc.EntryLnth - sizeof(ADI_ETHER_BF526_TRACE_ENTRY);
                dev->TraceFirstByte = trcinfo->FirstByte;
            }

            break;
        case ADI_ETHER_CMD_BF526_GET_TRACE:
            memcpy(pArg,&dev->Trc,sizeof(ADI_ETHER_BF526_TRACE_DATA));
            break;
...
static ADI_INT_HANDLER(DmaErrorInterruptHandler)
{
    ...
    ADI_ETHER_BF526_DATA *dev = (ADI_ETHER_BF526_DATA *)ClientArg;
    BUFFER_INFO *bi;
    int addr;
...
static  ADI_INT_HANDLER(EtherInterruptHandler)
{
    ...
    if (dev->DCBHandle) {
#ifdef ADI_ETHER_BF526_DEBUG
        ADI_ETHER_BF526_OutstandingPosts++;
#endif
...
static ADI_INT_HANDLER_RESULT ProcessCompletion(ADI_ETHER_BF526_DATA *dev, FRAME_QUEUE *q)
{
    ADI_INT_HANDLER_RESULT result = ADI_INT_RESULT_NOT_PROCESSED;
    ...
    while (act!=NULL) {
        ...
        *((long long *)&act->IPHdrChksum) = *((long long *)sts);
        ...
        if (dev->MaxTraceEntries>0) {
            ...
            ADI_ETHER_BF526_TRACE_ENTRY *te = dev->Trc.OldestEntry;
    ...
    if (lst!=NULL) {
        ...
        if (dev->DMCallback!= NULL) {
            ...
            if (dev->DCBHandle) {
#ifdef ADI_ETHER_BF526_DEBUG
                ADI_ETHER_BF526_PostedBuffer = act->CallbackParameter;
                ADI_ETHER_BF526_OutstandingPosts++;
#endif
...
#ifdef ADI_ETHER_BF526_DEBUG
                    ADI_ETHER_BF526_NoPosted += noposted;
#endif
                    if (q->Rcve) {
                        NoRxPosts++;
                    } else {
