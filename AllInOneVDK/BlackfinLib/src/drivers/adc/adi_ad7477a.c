...
$RCSfile: adi_ad7477a.c,v $
$Revision: 2095 $
$Date: 2010-02-18 17:08:05 -0500 (Thu, 18 Feb 2010) $

Description:
			This is the driver source code for the AD7477A ADC peripheral. It is layered
			on top of the SPI device driver, which is configured for the specific use of
			the AD7477A peripheral.
...
#define ADI_AD7477A_MODULE

#include <services/services.h>			// system service includes

#ifndef __ADSP_BRAEMAR__
#error "AD7477A driver is not available for the current processor"
#endif

#include <drivers/adi_dev.h>			// device manager includes
#include <drivers/spi/adi_spi.h>       	// spi driver includes
#include <drivers/adc/adi_ad7477a.h>	// AD7477A driver includes
...
#define ADI_AD7477A_NUM_DEVICES	(sizeof(Device)/sizeof(ADI_AD7477A))	// number of AD7477A devices in the system
...
typedef struct {
	...
	//ADI_AD7477A_OPERATION_MODE	CurMode;			// the Current mode of operation of the AD7477A device
...
// The prototpe functions for the AD7477A device driver

static u32 adi_pdd_Open(					// Open the AD7477A device
...
static u32 adi_pdd_Close(		// Closes the AD7477A device
...
ADI_DEV_PDD_ENTRY_POINT ADI_AD7477A_EntryPoint = {
	adi_pdd_Open,
	adi_pdd_Close,
	adi_pdd_Read,
	adi_pdd_Write,
...
*
*	Description:	Opens the AD7477A device for use
*
*********************************************************************/
...
static void spiCallbackFunction(void* DeviceHandle, u32 Event, void* pArg)
{
	ADI_AD7477A *pAD7477A = (ADI_AD7477A *)DeviceHandle;
#if defined(ADI_DEV_DEBUG)
	...
	(pAD7477A->DMCallback)(pAD7477A->DMHandle,Event,pArg);
}
...
static int ValidatePDDHandle(ADI_DEV_PDD_HANDLE PDDHandle) {
	unsigned int i;
	for (i = 0; i < ADI_AD7477A_NUM_DEVICES; i++) {
