...
#define ADI_AD7477_MODULE

#include <services/services.h>			// system service includes

#ifndef __ADSP_BRAEMAR__
#error "AD7477 driver is not available for the current processor"
#endif

#include <drivers/adi_dev.h>			// device manager includes
#include <drivers/spi/adi_spi.h>       	// spi driver includes
#include <drivers/adc/adi_ad7477.h>	// AD7477 driver includes
...
#define ADI_AD7477_NUM_DEVICES	(sizeof(Device)/sizeof(ADI_AD7477))	// number of AD7477 devices in the system

// Structure to contain data for an instance of the AD7477 device driver
#pragma pack(4)
typedef struct {
	...
	//ADI_AD7477_OPERATION_MODE	CurMode;			// the Current mode of operation of the AD7477 device
...
static ADI_AD7477 Device[] = {
	{
		NULL,
		NULL,
		0,
//		ADI_AD7477_MODE_NORMAL,
		NULL,
		1062,
	},
};

// The prototpe functions for the AD7477 device driver

static u32 adi_pdd_Open(					// Open the AD7477 device
...
static u32 adi_pdd_Close(		// Closes the AD7477 device
...
ADI_DEV_PDD_ENTRY_POINT ADI_AD7477_EntryPoint = {
	adi_pdd_Open,
	adi_pdd_Close,
	adi_pdd_Read,
	adi_pdd_Write,
...
*
*	Description:	Opens the AD7477 device for use
*
*********************************************************************/
...
static u32 adi_pdd_Open(
	...
	ADI_AD7477 *pAD7477;		// pointer to the AD7477 device we're working on
	void *pExitCriticalArg;		// exit critical region parameter
	
	// Check for a valid device number and that we're only doing input
#ifdef ADI_DEV_DEBUG
	if (DeviceNumber >= ADI_AD7477_NUM_DEVICES) return (ADI_DEV_RESULT_BAD_DEVICE_NUMBER);
	...
	pAD7477 = &Device[DeviceNumber];
	// and store the Device Manager handle 
	pAD7477->DMHandle = DMHandle;
	// and callback function
	pAD7477->DMCallback = DMCallback;
	...
	//pAD7477->CurMode = ADI_AD7477_MODE_NORMAL;
	pExitCriticalArg = adi_int_EnterCriticalRegion(pEnterCriticalArg);
	if (pAD7477->InUseFlag == FALSE) {
		pAD7477->InUseFlag = TRUE;
	...
	Result = adi_dev_Open(
					...
					0,						// device number
					pAD7477,				// client handle - passed to internal callback function
					&pAD7477->spiHandle,	// pointer to DM handle (for SPI driver) location 
	...
	// appropriate values for the AD7477 device
	ADI_DEV_CMD_VALUE_PAIR SPI_config[] = {
		...
		{ ADI_SPI_CMD_SET_WORD_SIZE, (void*)16 },			// Data word size 16 Bit (we need 16bits
															// since the AD7477 device generates 10 bits data with 4 leading
	...
	Result = adi_dev_Control( pAD7477->spiHandle, ADI_DEV_CMD_TABLE, (void*)SPI_config );
#ifdef ADI_DEV_DEBUG
	if (Result != ADI_DEV_RESULT_SUCCESS) return (Result);
#endif

	// save the physical device handle in the client supplied location
	*pPDDHandle = (ADI_DEV_PDD_HANDLE *)pAD7477;
...
static u32 adi_pdd_Close(		// Closes a device
	...
	ADI_AD7477 *pAD7477 = (ADI_AD7477 *)PDDHandle;
	...
	// close SPI driver
	Result = adi_dev_Close(pAD7477->spiHandle);
#ifdef ADI_DEV_DEBUG
	if (Result != ADI_DEV_RESULT_SUCCESS) return (Result);
#endif

	// mark the device as closed
	pAD7477->InUseFlag = FALSE;
...
static u32 adi_pdd_Read(		// Reads data or queues an inbound buffer to a device
	...
	ADI_AD7477 *pAD7477 = (ADI_AD7477 *)PDDHandle;
	Result = adi_dev_Read(pAD7477->spiHandle,BufferType,pBuffer);
...
static u32 adi_pdd_Control(		// Sets or senses a device specific parameter
	...
	ADI_AD7477 				*pAD7477;		// pointer to the device we're working on
	...
	// avoid casts
	pAD7477 = (ADI_AD7477 *)PDDHandle; // Pointer to AD7477 device driver instance
	...
	switch (Command) 
	{
		...
		// CASE (Set the sample rate of the AD7477 device)
		case (ADI_AD7477_CMD_SET_SAMPLE_RATE):
			{
				...
				// used. NB. fsclk ~ O(1E8), pAD7477->tquiet_factor # O(1000), so that x will 
				// be at least 0(1E4).
				u32 y = 32*pAD7477->tquiet_factor;
				u32 x = fsclk/y;
				...
				Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_SET_BAUD_REG, (void*)spi_baud);
		...
		case (ADI_AD7477_CMD_SET_OPERATION_MODE):
			
			// IF (powering down the device)
			//if (u32Value==ADI_AD7477_MODE_POWERDOWN && pAD7477->CurMode==ADI_AD7477_MODE_NORMAL)
			if (u32Value == ADI_AD7477_MODE_POWERDOWN) {
				
				// pause SPI dataflow
				if (Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_PAUSE_DATAFLOW, (void*)TRUE)) break;
				
				// execute a dummy 8 bit read to force the part into powerdown
				if (Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_EXECUTE_DUMMY_READ, (void*)8)) break;
				
			// ELSE 
			//} else if (u32Value==ADI_AD7477_MODE_NORMAL && pAD7477->CurMode==ADI_AD7477_MODE_POWERDOWN)  {
			} else {
				
				// execute a dummy 16 bit read to force the part to power up
				if (Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_EXECUTE_DUMMY_READ, (void*)16)) break;
				
				// resume SPI dataflow
				if (Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_PAUSE_DATAFLOW, (void*)FALSE)) break;
		...
		case (ADI_AD7477_CMD_DECLARE_SLAVE_SELECT):
			...
			Result = adi_dev_Control(pAD7477->spiHandle, ADI_SPI_CMD_ENABLE_SLAVE_SELECT, Value);
			break;
		
		// CASE (replace the default quiet period factor)
		case (ADI_AD7477_CMD_SET_QUIET_PERIOD_FACTOR):
			...
			pAD7477->tquiet_factor = u32Value;

		// DEFAULT 
		default:
		
			// pass anything we don't specifically handle to the SPI driver
			Result = adi_dev_Control(pAD7477->spiHandle,Command,Value);
			break;
...
static void spiCallbackFunction(void* DeviceHandle, u32 Event, void* pArg)
{
	ADI_AD7477 *pAD7477 = (ADI_AD7477 *)DeviceHandle;
#if defined(ADI_DEV_DEBUG)
	...
	(pAD7477->DMCallback)(pAD7477->DMHandle,Event,pArg);
}
...
static int ValidatePDDHandle(ADI_DEV_PDD_HANDLE PDDHandle) {
	unsigned int i;
	for (i = 0; i < ADI_AD7477_NUM_DEVICES; i++) {
