...
unsigned int numTicks=0;
    ...
    /* case to check whether semaphore is posted or not without blocking 
     * un-avaibility of the semaphore is indicated via ker_err_sem_unavailable
     */
    if(timeout == VDK_kDoNotWait)
    {
    	/* pend returns FALSE in case semaphore is not posted yet (count is 0) */
    	if (!VDK_PendSemaphore((VDK_SemaphoreID)(sem_id),timeout))
    	   return ker_err_sem_unavailable;
    	else
    	   return (0);
    }
 ...
 *
 ##########################################################################*/
int ker_get_thread_status(int id)
{
    if(VDK_GetThreadStatus((VDK_ThreadID)id) == VDK_kUnknown)
       return (-1);
    
    // TODO: return an enumerated value of the thread state  
    return (0);
}
