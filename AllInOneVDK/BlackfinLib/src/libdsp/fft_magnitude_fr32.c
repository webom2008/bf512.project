/******************************************************************************
 *
 * fft_magnitude_fr32.c :  $Revision: 1.3 $
 *
 ...
 *****************************************************************************/
    ...
    Description:
        ...
        The fft_magnitude_fr32 function computes a normalized power spectrum
        ...
        to 0 if the input was generated by the library functions cfft_fr32
        or cfftf_fr32 and mode must be set 1 if the input was generated by
        the rfft_fr32 or rfftf_fr32 library functions.
        ...
        is returned by the cfft_fr32 and rfft_fr32 functions. If on the other
        hand the input array was generated by the cfftf_fr32 or rfftf_fr32
        functions then the block_exponent argument should be set to -1, which
        indicates that the input array was generated using static scaling.
        ...
        Note: Functions that compute an FFT using fixed-point arithmetic
              ...
              description of the cfft_fr32 or rfft_fr32 functions for more
              information about different scaling methods.

        The fft_magnitude_fr32 function writes the power spectrum to the
    ...
    Error Conditions:
        ...
        The function fft_magnitude_fr32 exits without modifying the output
    ...
    Implementation:
        ...
        The function makes use of the shr_fr1x32 and norm_fr1x32 functions
        (defined in fract_math.h) as well as the cabs_fr32 function (defined
        in complex.h).

        Instead of a division by fft_size, the output from the cabs_fr32
        function is shifted right by log2(fft_size). Using the shr_fr1x32
        ...
        affected by the various scaling modes available in the cfft_fr32
        and rfft_fr32 functions:
    ...
    Example:
        ...
        fract32           real_signal[N_FFT];
        complex_fract32   cplx_signal[N_FFT];

        complex_fract32   fft_output[N_FFT];

        complex_fract32   twiddle_table[N_FFT];

        fract32           real_magnitude[(N_FFT/2)+1];
        fract32           cplx_magnitude[N_FFT];
        ...
        rfft_fr32 (real_signal, fft_output, twiddle_table,
                   1, N_FFT, &block_exponent, 2);

        fft_magnitude_fr32 (fft_output, real_magnitude,
                   N_FFT, block_exponent, 1);


        twidfftf_fr32 (twiddle_table, N_FFT);

        cfftf_fr32 (cplx_signal, fft_output, twiddle_table,
                    1, N_FFT);

        fft_magnitude_fr32 (fft_output, cplx_magnitude,
                    N_FFT, -1, 0);
...
#pragma file_attr("libFunc  = fft_magnitude_fr32")
#pragma file_attr("libFunc  = __fft_magnitude_fr32")
...
extern void
fft_magnitude_fr32 (const complex_fract32  _input[],
                    fract32                _output[],
                    int                    _fft_size,
                    int                    _block_exponent,
                    int                    _mode)
{
    ...
    fract32 *__restrict             output = _output;
...
#pragma loop_count(2,,)
    ...
    */
    for (i = 0; i < max_i; i++)
    {
       output[i] = shr_fr1x32 (cabs_fr32 (input[i]), scale_exponent);
    }   
}
