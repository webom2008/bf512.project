 ...
 ************************************************************************/
    ...
    Function: vecmax_fr32 - vector maximum

    Synopsis:

        #include <vector.h>
        fract32 vecmax_fr32 (const fract32 vector[],
                             int           length);

    Description:

        The vecmax_fr32 function returns the largest value in the
        given vector, vector[]. The parameter length defines the size
        of the input vector.

    Error Conditions:

        The vecmax_fr32 function is an effective no-op if the parameter
        length is less than or equal to zero.

    Algorithm:

        vecmax_fr32(vector,length) = maximum(vector[0],..,vector[length-1])
    ...
    Example:
        ...
        max_val = vecmax_fr32 (input,SAMPLES_N);
...
#pragma file_attr("libFunc  =vecmax_fr32")
#pragma file_attr("libFunc  =__vecmax_fr32")
...
extern fract32
vecmax_fr32 (const fract32 _vector[],
             int           _length)
{
...
#pragma vector_for
    ...
    for (i = 1; i < length; i++)
    {
        max_val = max_fr1x32(vector[i],max_val);
    }
