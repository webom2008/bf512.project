/************************************************************************
 *
 * crosscorr_fr32.c : $Revision: 1.7.4.1 $
 *
 ...
 ************************************************************************/
    ...
    Function: crosscorr_fr32 - cross correlation
    ...
    Description:
        ...
        The crosscorr_fr32 performs a cross-correlation between two
        signals. The cross-correlation is the sum of the scalar products
        of the signals in which the signals are displaced in time with
        respect to one another. The signals to be correlated are given
        by the input vectors samples_x[] and samples_y[]. The length of
        the input vectors is given by sample_length.

        The function returns the result in the array correlation with
        lags elements.

        Cross-correlation is used in signal processing applications such
        as speech analysis.

    Error Conditions:

        The crosscorr_fr32 function is an effective no-op if the parameter
        sample_length or lags is less than or equal to zero.

    Algorithm:

        correlation[k] = (1 / sample_length) *
                         Sigma(i = 0 to <sample_length>-k-1)
                         samples_x[i] * samples_y[i+k]
    ...
    Example:
        ...
        #define SAMPLE_LENGTH 45
        #define LAGS  10

        fract32 input_x[SAMPLE_LENGTH];
        fract32 input_y[SAMPLE_LENGTH];
        fract32 output[LAGS];

        crosscorr_fr32(input_x, input_y, SAMPLE_LENGTH, LAGS, output);
...
#pragma file_attr("libFunc  =crosscorr_fr32")
#pragma file_attr("libFunc  =__crosscorr_fr32")
...
extern void
crosscorr_fr32(const fract32 _samples_x[],
