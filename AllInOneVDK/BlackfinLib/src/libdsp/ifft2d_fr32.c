/****************************************************************************
 *
 * ifft2d_fr32.c : $Revision: 1.7.12.1 $
 *
 ...
 ***************************************************************************/
    ...
    Function: ifft2d_fr32 - inverse N x N point 2-D Fast Fourier Transform 
    ...
    Description:
        ...
        This function computes the two-dimensional inverse Fast Fourier 
        Transform (FFT) of the complex input matrix input[fft_size][fft_size] 
        and stores the result in the complex output matrix 
        ...
        achieved by also specifying the input matrix as the output buffer.
    ...
    Error Conditions:
        ...
        The ifft2d_fr32 function aborts if fft_size is less than 8 
    ...
    Implementation:
        ...
        of the ifftf_fr32 and ctranspm_fr32 DSP runtime library functions.
    ...
    Example:
        ...
        ifft2d_fr32 (in1, temp, in1, twiddle, 
                     TWIDDLE_STRIDE1, FFT_SIZE1);

        ifft2d_fr32 (in2, temp, out2, twiddle,
...
#pragma file_attr("libFunc  =ifft2d_fr32")
#pragma file_attr("libFunc  =__ifft2d_fr32")
...
extern void  
ifft2d_fr32 (const complex_fract32  *_input,
...
#pragma loop_count(8,,)
    ...
    for (j = 0; j < rows; j++)
    {
        ifftf_fr32 (&input[j*columns], &temp[j*columns], 
...
#pragma loop_count(8,,)
    ...
    for (j = 0; j < columns; j++)
    {
        ifftf_fr32 (&output[j*rows], &temp[j*rows],
