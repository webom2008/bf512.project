/************************************************************************
 *
 * atan2d.c : $Revision: 4 $
 *
 * (c) Copyright 2004 Analog Devices, Inc.  All rights reserved.
 *
 ************************************************************************/

#pragma file_attr("libGroup =math.h")
#pragma file_attr("libFunc  =__atan2d")
#pragma file_attr("libFunc  =atan2d")
#pragma file_attr("libFunc  =atan2l")
#pragma file_attr("libFunc  =atan2")
/* Called by argd */
#pragma file_attr("libGroup =complex.h")
#pragma file_attr("libFunc  =__argd")
#pragma file_attr("libFunc  =argd")
#pragma file_attr("libFunc  =arg")
/* argd: called by cartesiand */
#pragma file_attr("libFunc  =cartesiand")
#pragma file_attr("libFunc  =__cartesiand")
#pragma file_attr("libFunc  =cartesian")
#pragma file_attr("libName =libdsp")
#pragma file_attr("prefersMem =internal")
#pragma file_attr("prefersMemNum =30")

/*
 * Description    : This file contains the 64-bit implementation of atan2().
 *                  The algorithm used to implement this function is adapted 
 *                  from Cody & Waite, "Software Manual for the Elementary 
 *                  Functions", Prentice-Hall, New Jersey, 1980.
 *
 *                  Domain      : x = Full Range, y = Full Range, 
 *                                NOT x = y = 0.0
 *                                     This function returns 0 for
 *                                     error input of x = y = 0
 *                  Accuracy    : 3 bits or less for entire range
 */

/* Defined in */
#include <math.h>

#include "math_const.h"
#include "util.h"


DOUBLE                            /*{ ret - atan(y/x)    }*/
atan2d
(
  DOUBLE y,                       /*{ (i) - y coordinate }*/
  DOUBLE x                        /*{ (i) - x coordinate }*/
)
{
    DOUBLE f, g;
    DOUBLE num, den;
    DOUBLE result;
    int n;
    static const DOUBLE a[4] = {0, (DOUBLE)PI_6, (DOUBLE)PI_2, (DOUBLE)PI_3};

    /*{ Check for x == 0 }*/
    if (x == 0.0)
    {
        /*{ if y == 0, return error }*/
        if (y == 0.0)
        {
            result = 0.0;
            return result;
        }

        result = (DOUBLE)PI_2;

        /*{ if y > 0, return PI/2 }*/
        if (y > 0.0)
        {
            return result;
        }

        /*{ if y < 0, return -PI/2 }*/
        if (y < 0.0)
        {
            result = -result;
            return result;
        }
    }

    /*{ n = 0 }*/
    n = 0;

    /*{ num = |y| }*/
    /*{ den = |x| }*/
    num = y;
    den = x;
    if (num < 0.0)
    {
        num = -num;
    }
    if (den < 0.0)
    {
        den = -den;
    }

    /*{ if num > den }*/
    if (num > den)
    {
        /*{ swap den and num }*/
        f = den;
        den = num;
        num = f;
        /*{ n = 2 }*/
        n = 2;
    }
    /*{ f = num/den }*/
    f = DIVD(num, den);

    /*{ if f > 2 - sqrt(3) }*/
    if (f > TWO_MINUS_ROOT3)
    {
        /*{ f = [f * sqrt(3) - 1] / [sqrt(3) + f] }*/
        num = MPYD(SQRT3_MINUS_1, f);
        num = SUBD(num, 0.5);
        num = SUBD(num, 0.5);
        num = ADDD(num, f);
        den = ADDD(SQRT3, f);
        f = DIVD(num, den);

        /*{ n = n + 1 }*/
        n = n + 1;
    }

    g = f;
    if (g < 0.0)
    {
        g = -g;
    }

    /*{ if |f| < eps }*/
    if (g < EPS_DOUBLE)
    {
        /*{ result = f }*/
        result = f;
    }
    /*{ else |f| >= eps }*/
    else
    {
        /*{ g = f * f }*/
        g = MPYD(f, f);
    
        /*{ result = R(g) = g * P(g) / Q(g) }*/
        /*{!INDENT}*/
        /*{ P(g) = ((p3 * g + p2) * g + p1) * g + p0 }*/
        num = MPYD(ATANDP_COEF3, g);
        num = ADDD(num, ATANDP_COEF2);
        num = MPYD(num, g);
        num = ADDD(num, ATANDP_COEF1);
        num = MPYD(num, g);
        num = ADDD(num, ATANDP_COEF0);
        num = MPYD(num, g);
    
        /*{ Q(g) = (((g + q3) * g + q2) * g + q1) * g + q0 }*/
        den = ADDD(g, ATANDQ_COEF3);
        den = MPYD(den, g);
        den = ADDD(den, ATANDQ_COEF2);
        den = MPYD(den, g);
        den = ADDD(den, ATANDQ_COEF1);
        den = MPYD(den, g);
        den = ADDD(den, ATANDQ_COEF0);
    
        result = DIVD(num, den);
        /*{!OUTDENT}*/
    
        /*{ result = result * f + f }*/
        result = MPYD(result, f);
        result = ADDD(result, f);
    }
    
    /*{ n > 1, result = -result }*/
    if (n > 1)
    {
        result = -result;
    }
    
    /*{ result = result + a[n] }*/
    result = ADDD(a[n], result);

    /*{ if x < 0, result = PI - result }*/
    if (x < 0.0)
    {
        result = SUBD(PI, result);
    }

    /*{ if y < 0, result = -result }*/
    if (y < 0.0)
    {
        result = -result;
    }

    /*{ return result }*/
    return result;
}
