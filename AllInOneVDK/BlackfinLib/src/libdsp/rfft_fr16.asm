 ...
 ******************************************************************************
  ...
  Implementation : radix-2 real Fast Fourier Transform
  ...
  Algorithm      : The computation of the FFT is split into three parts.
                   ...
                   in-place FFT). 
                   Bit-reversing is performed on real data, thus the data
                   written will still be of type fract16 (not complex_fract16).
                   This is neccessary for in-place bit-reversal to work (since
                   otherwise memory written to as complex_fract16 will be 
                   consumed faster than can it can be read (as fract16).                    
                   ...
                   greatly simplified.
                   The input data at this stage are of type fract16. The 
                   data are read in reverse order, starting at location 
                   ((fft_size / 2) - 1). The output data of type 
                   complex_fract16 are written in reverse order, starting
                   at location (fft_size - 1).
  ...
  Cycle count    : Using the cycle accurate ADSP-BF532 simulator without any
                   ...
                   Size FFT     Cycle Count 
                        ...
                        128        2210           2189           2968
                        256        4460           4436           6247
                       1024       19715          19685          28921

                   * cycle counts for dynamic scaling are dependent on
                     the input data
                     [for a 1024 point fft:  28900 (always scale)
                                             28936 (never scale)]
...
.file_attr libFunc       = __rfftN_scaling_fr16;
.file_attr libFunc       = rfftN_scaling_fr16;
.file_attr FuncName      = __rfftN_scaling_fr16;
#endif
...
.section program;
.global  __rfftN_scaling_fr16;


.align 2;
__rfftN_scaling_fr16:
...
.dynamic:
        ...
        R1 = R1 -|- R1
        ||[SP+_INPUT_STRIDE] = R3;       // Save initial offset (=8)
        ...
        M0 = R6;                       // Size FFT (for bit-reversal)
...
/** Bit reversal **/
        ...
        /* Loop for FFT size 
         *   Copy fract16 data from the input buffer into the output buffer 
         *   in bit-reversed order and store them as fract16 data. 
...
.bitrev_start:
           I0 = I3;
           R7.L = EXPADJ (R0.L, R7.L ) || I0 += M2 || R0.L = W[I2++];
           I3 += M0 (BREV) || W[I0] = R0.L;
           I0 = I3;
           R7.L = EXPADJ (R0.L, R7.L ) || I0 += M2 || R0.L = W[I2++];
.bitrev_end:
           I3 += M0 (BREV) || W[I0] = R0.L;

        R7.L = EXPADJ (R0.L, R7.L );
        JUMP .end_bitrev;
...
.bitrev_in_place:
        ...
        /* Loop for FFT size
         *   Re-arrange fract16 data in the input buffer in 
         *   bit-reversed order.
         *   At the same time, scan the input data to detect whether
         *   dynamic scaling is required for the first stage.
         *
...
.skip_swap_op:
           ...
           */
           R7.L = EXPADJ (R0.L, R7.L ) || I3 += M0 (BREV) || R0.L = W[I2++];
...
/** First stage **/
        ...
        R2 = R2 -|- R2  
        || R3.L = W[I3--];             // Load data[(fft_size / 2) - 1]
    
        R1.L = R1.L + R6.L (S)         // Increment count block exponent
        || R2.L = W[I3--]              // and load data[(fft_size / 2) - 2]
        || R7 = [I0++M2];              // and position write pointer

        R4 = R2 +|+ R3, R5 = R2 -|- R3;// Zero R4, R5
        ...
        /* Loop N/2 times 
             ...
             Read fract16 data in reverse order and store
             them as complex_fract16 data in reverse order.
             Scale input values 
         */
...
.first_s_end:
           ...
           R3 = R3 >>> 1 (V) || R4.L = W[I3--];
           R6 = R2 +|+ R3, R7 = R2 -|- R3;
           R4 = R4 >>> 1 (V) || [I0--] = R7 || R3.L = W[I3--];
           R5 = R5 >>> 1 (V) || [I0--] = R6 || R2.L = W[I3--];
           R6 = R4 +|+ R5, R7 = R4 -|- R5;
           [I0--] = R7;
.first_s_end:
           [I0--] = R6;
...
.remaining_stages:
        ...
        [SP+_CONST_FOUR] = P1
        || R5 = [I0++M2];              // Position write pointer (dummy read)

        R6 = [SP+_INPUT_STRIDE]        // Copy offset input into register R6        
        || R5 = [I3++M2];              // Position read pointer (dummy read) 
...
.end_traverse:
        ...
        || [SP+_INPUT_STRIDE] = R6;    // Update offset input 
...
.end_other_ns_stages:
        ...
        || R5 = [I3++M2] || I0 += M2;  // Position read, write pointer
                                       // using dummy read
