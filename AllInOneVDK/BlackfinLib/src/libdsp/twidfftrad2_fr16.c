/*****************************************************************************
 *
 * twidfftrad2_fr16 : $Revision: 3543 $
 *
 ...
 *****************************************************************************/
    ...
    Function: twidfftrad2_fr16 - twiddle factor generator
    ...
    Description:
        ...
        The twidfftrad2_fr16 function calculates complex twiddle coefficients
        for an FFT of size fft_size and returns the coefficients in the vector
        twiddle_table. The size of the vector, which is known as a twiddle
        table, must be at least fft_size/2. It contains pairs of sine and
        cosine values that are used by an FFT function to calculate a Fast
        Fourier Transform. The table generated by this function may be used
        by any of the FFT functions cfft_fr16, ifft_fr16, and rfft_fr16.
        ...
        used to compute the FFTs provided that the table is created for the
        largest FFT that the application expects to generate. Each of the FFT
        functions cfft_fr16, ifft_fr16, and rfft_fr16 have a twiddle stride
        argument that the application would set to 1 when it is generating
        an FFT with the largest number of data points. To generate smaller
        FFTs, the twiddle stride argument should be set according to the
        formula:
    ...
    Error Conditions:
        ...
        The twidfftrad2_fr16 function does not return an error condition.
    ...
    Example:
        ...
        complex_fract16  input1[FFT_SIZE1], output1[FFT_SIZE1];
        complex_fract16  input2[FFT_SIZE2], output2[FFT_SIZE2];
        complex_fract16  twiddles[TWID_SIZE];

        int              block_exponent1, block_exponent2;
        int              scale_method = 1;

        twidfftrad2_fr16 (twiddles, FFT_SIZE1);

        /* Generate different sized FFTs with the same twiddle table */

        cfft_fr16 (input1, output1, twiddles,
                   1, FFT_SIZE1,
                   &block_exponent1, scale_method);

        cfft_fr16 (input2, output2, twiddles,
...
#pragma file_attr ("libFunc  =twidfftrad2_fr16")
#pragma file_attr ("libFunc  =__twidfftrad2_fr16")
...
extern void
twidfftrad2_fr16( complex_fract16 _twiddle_table[],
                  int             _fft_size )
{
