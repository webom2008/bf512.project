/************************************************************************
 *
 * gen_hanning_fr32.c : $Revision: 1.5 $
 *
 ...
 ************************************************************************/
    ...
    Function: gen_hanning_fr32 - Hanning Window Generator
    ...
    Description:
        ...
        values within the output vector hanning_window. The length of the
        output vector should therefore be window_size * window_stride.

        This window is also known as either Hann window or cosine window.

    Error Conditions:

        The function gen_hanning_fr32 exits without modifying the output
    ...
    Implementation:
        ...
        arithmetic:
            ...
            0x40000000 - (0x40000000 * __cos32_2PIx (i * (1/(n-1)))) =
            0x40000000 - (__cos32_2PIx (i * (1/(n-1))) >> 1)
        ...
        The value of the first / last element is fixed at:
            ...
            0.5 - (0.5 * cos ((2*pi*0)/(n-1))) = 0.5 - 0.5 * cos(0) = 0

        For odd sample length, the mid point is fixed at:

            0.5 - (0.5 * cos ((2*pi*((n-1)/2))/(n-1))) =
            0.5 - (0.5 * cos ((((2*pi*n)-(2*pi))/2)/(n-1))) =
            0.5 - (0.5 * cos (((pi*n)-pi)/(n-1))) =
            0.5 - (0.5 * cos (((pi*(n-1))/(n-1))) =
            0.5 - (0.5 * cos (pi)) = 0.5 - (0.5 * -1) = 1
    ...
    Example:
        ...
        fract32           hanning_window[N_FFT];

        int block_exponent;


        gen_hanning_fr32 (hanning_window, 1, N_FFT);

        vecvmlt_fr32 (signal, hanning_window,
...
extern void
gen_hanning_fr32 (fract32  _hanning_window[],
                  int      _window_stride,
                  int      _window_size)
{
    /* local copies of the arguments */

    fract32 *__restrict  hanning_window = _hanning_window;
    ...
    if expected_true ((window_size > 1) && (window_stride > 0))
    {
        ...
        */
        hanning_window[n_half*window_stride] = 0x7FFFFFFF;

        /* set first and last value in table to default */
        offset_lowhalf = 0;
        offset_highhalf = (window_size - 1) * window_stride;
        hanning_window[offset_lowhalf] = 0x0;
        hanning_window[offset_highhalf] = 0x0;
...
#pragma vector_for
        for (i = 1; i < n_half; i++)
        {
            ...
            tmp = __cos32_2PIx (alpha) >> 1;

            win_value = sub_fr1x32 (0x40000000, tmp);

            offset_lowhalf += window_stride;
            offset_highhalf -= window_stride;

            hanning_window[offset_lowhalf] = win_value;
            hanning_window[offset_highhalf] = win_value;
        }
    }
