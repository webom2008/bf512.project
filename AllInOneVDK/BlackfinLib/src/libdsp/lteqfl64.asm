...
** floating-point less than or equal comparison. This functions is for
** compiler internal use only. It does not follow the normal C ABI.
**
** int __float64_adi_lteq(long double X, long double Y)
**
...
%rtl "12fcmp <="
%notes "Double-precision floating-point less-than-or-equal-to comparison."
%regs_clobbered "R0-R3,P0-P2,CCset"
%const
%return_in_cc
%syscall ___float64_adi_lteq(I=lDlD)
#endif
...
.file_attr libFunc = ___float64_adi_lteq;
.file_attr FuncName      = ___float64_adi_lteq;
#endif
...
___float64_adi_lteq:
	...
	IF !CC JUMP .ret2;		// it'll do as the return limit
	...
	R4 = R1 - R3;
	CC &= AZ;
	IF CC JUMP .ret2;		// X==Y so return CC=1
	...
	R4 <<= 1;
	CC &= AZ;
	IF CC JUMP .ret2;		// -0==+0 so return CC=1
	...
	CC = !CC;				// which means that X > Y
	R5 = R1 & R3;			// so invert to form the result
...
.___float64_adi_lteq.end:
.global ___float64_adi_lteq;
.type ___float64_adi_lteq, STT_FUNC;
