...
**
** int __float64_adi_gteq(long double X, long double Y)
**
...
%notes "Double-precision floating-point greater-than-or-equal-to comparison."
%const
%return_in_cc
%syscall ___float64_adi_gteq(I=lDlD)
#endif
...
___float64_adi_gteq:
	...
	R4 = R1 - R3;
	CC &= AZ;
	IF CC JUMP .ret1;	// X=Y so return CC=1
	...
	R4 <<= 1;
	CC &= AZ;
	IF CC JUMP .ret1;	// -0=+0 so return CC=1
