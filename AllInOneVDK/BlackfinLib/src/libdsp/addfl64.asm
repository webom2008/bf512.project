...
**
** Denormal inputs and results are flushed to a positive 0.0 result.
**
...
%rtl "12f+"
%notes "Double-precision floating-point addition."
...
%const
%syscall ___float64_add(lD=lDlD)
#endif
...
___float64_add_inregs:
   ...
   R4 = R1 << 1;        // Remove sign of X
   R6 = R4 >> 21;       // Extract X exponent

   R5 = R3 << 1;        // Remove sign of Y
   R7 = R5 >> 21;       // Extract Y exponent

   CC = R6 == 0;        // If X exponent is 0 (0.0 or denormal) return Y
   IF CC JUMP .return_y;

   CC = R7 == 0;        // If Y exponent is 0 (0.0 or denormal), return X
   IF CC JUMP .return_x_nozcheck;

   P1 = R4;             // hold X without sign (high)
   ...
   /* if we get here inputs are normal and not Inf, NaN or 0.0 */
...
.no_swap:
   ...
   R5.H = 1;
   R5.L = (20 << 8) | 12;
   CC = P1 < 0;            // here to avoid anomaly 05000209
   R1 = DEPOSIT(R1, R5);
   R3 = DEPOSIT(R3, R5);
...
.normalizing_left:
   CC = R6 < R5;             // if normalizing shift greater than
                             // eR, we need to return denormalized.
   IF CC JUMP .return_zero;
   
   R4 = R7 - R5;

   // Once we've moved the one-bit into posn 20 of R1
...
#if defined(__WORKAROUND_SHIFT) 
   M0 = R6;
   R6 = 0; 
   // LSHIFT uses 7 bits of the operand on BF535 cores
   R4 = -R4;
   R7 = LSHIFT R7 BY R4.L;
   CC = R5 < 0;
...
.normalized:
   ...
   R1 = R1 + R7;             // and ripple up
	R2 = R0 | R1;
	CC = R2 == 0;
	if CC JUMP .rtn;          // avoid returning -0.0
.sign_and_rtn:
...
.handle_nan_inf_x:
	...
	IF CC JUMP .return_x_nozcheck;
#if WA_05000428
   // avoid a speculative load in the three instructions after a 
   // conditional branch to workaround BF561 anomaly 05000428
   NOP;   
#endif
	R0 = -1;				// Otherwise return default NaN 
	R1 = -1;
   (R7:4) = [SP++];
   RTS;
 
.return_y:
   // avoid returning -0.0 or denormal results
	CC = R7 == 0;
	IF CC jump .return_zero;
#if WA_05000428
   // avoid a speculative load in the three instructions after a 
