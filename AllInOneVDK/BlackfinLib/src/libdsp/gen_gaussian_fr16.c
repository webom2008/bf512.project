/************************************************************************
 *
 * gen_gaussian_fr16 : $Revision: 1.5 $
 *
 ...
 ************************************************************************/
    ...
    Function: gen_gaussian_fr16 - Gaussian Window Generator
    ...
    Error Conditions:
        ...
        The function gen_gaussian_fr16 exits without modifying the output
    ...
    Algorithm:
        ...
        Implementing the algorithm using 32-bit floating point arithmetic will 
        ensure that all intermediate results will preserve more significant 
        digits than can be stored in a variable of type fract16 in order to 
        maximize accuracy.
    ...
    Example:
        ...
        fract16           signal[N_FFT];

        fract16           windowed_signal[N_FFT];
        complex_fract16   fft_output[N_FFT];
        complex_fract16   twiddle_table[N_FFT/2];

        fract16           gaussian_window[N_FFT];

        int block_exponent;


        gen_gaussian_fr16 (gaussian_window, 0.7071068F, 1, N_FFT);

        vecvmlt_fr16 (signal, gaussian_window,
                      windowed_signal, N_FFT);

        twidfftrad2_fr16 (twiddle_table, N_FFT);

        rfft_fr16 (windowed_signal, fft_output, twiddle_table,
...
extern void
gen_gaussian_fr16 (fract16  _gaussian_window[],
                   float    _alpha,
                   int      _window_stride,
                   int      _window_size)
{
    ...
    fract16 *__restrict  gaussian_window = _gaussian_window;
    ...
    if (expected_true ((window_size > 0) && (window_stride > 0)))
    {
        ...
        */
        gaussian_window[n_half*window_stride] = 0x7FFF;

        offset_lowhalf = -window_stride;
        offset_highhalf = window_size * window_stride;

        alpha_over_n = (0.70710678118654752440F * alpha) / 
                       (float) window_size; 
...
#pragma vector_for
        for (i = 0; i < n_half; i++)
        {
            ...
            tmp = alpha_over_n * (float) moving_index;

            win_value = float_to_fr16 (expf (-tmp * tmp));
