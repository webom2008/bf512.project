/************************************************************************
 *
 * gen_blackman_fr32.c : $Revision: 1.5 $
 *
 ...
 ************************************************************************/
    ...
    Function: gen_blackman_fr32 - Blackman Window Generator
    ...
    Description:
        ...
        values within the output vector blackman_window. The length of the
        output vector should therefore be window_size * window_stride.

    Error Conditions:

        The function gen_blackman_fr32 exits without modifying the output
    ...
    Implementation:
        ...
        of the algorithm:
            ...
            0.42 - (0.50 * cos ((2*pi*i)/(n-1)))
                 + (0.08 * cos ((4*pi*i)/(n-1)))

                  = 0.42 - (0.50 * cos ((2*pi*(n-1-i))/(n-1)))
                         + (0.08 * cos ((4*pi*(n-1-i))/(n-1)))

                  = 0.42 - (0.50 * cos ((2*pi*((n-1)-(i)))/(n-1)))
                         + (0.08 * cos ((4*pi*(((n-1)-(i))/(n-1)))

                  = 0.42 - (0.50 * cos (((2*pi*(n-1))-(2*pi*i))/(n-1))
                         + (0.08 * cos (((4*pi*(n-1))-(4*pi*i))/(n-1))

                  = 0.42 - (0.50 * cos (((2*pi*(n-1))/(n-1))-((2*pi*i)/(n-1)))
                         + (0.08 * cos (((4*pi*(n-1))/(n-1))-((4*pi*i)/(n-1)))

                  = 0.42 - (0.50 * cos (2*pi - ((2*pi*i)/(n-1)))
                         + (0.08 * cos (4*pi - ((4*pi*i)/(n-1)))
        ...
        arithmetic:
            ...
            0x35c28f5c - (0x40000000 * __cos32_2PIx (i * (1/(n-1))))
                       + (0x0a3d70a4 * __cos32_2PIx ((2 * i) * (1/(n-1)))) =

            0x35c28f5c - (__cos32_2PIx (i * (1/(n-1))) >> 1)
                       + (0x0a3d70a4 * __cos32_2PIx ((2 * i) * (1/(n-1))))
        ...
        The value of the first / last element is fixed at:
            ...
            0.42 - (0.50 * cos ((2*pi*0)/(n-1)))
                 + (0.08 * cos ((4*pi*0)/(n-1))) =

            0.42 - (0.50 * cos (0)) + (0.08 * cos (0)) =

            0.42 - 0.50 + 0.08 = 0

        For odd sample length, the mid point is fixed at:

            0.42 - (0.50 * cos ((2*pi*((n-1)/2))/(n-1)))
                 + (0.08 * cos ((4*pi*((n-1)/2))/(n-1))) =

            0.42 - (0.50 * cos ((((2*pi*n)-(2*pi))/2)/(n-1)))
                 + (0.08 * cos ((((4*pi*n)-(4*pi))/2)/(n-1)))

            0.42 - (0.50 * cos (((1*pi*n)-(1*pi))/(n-1)))
                 + (0.08 * cos (((2*pi*n)-(2*pi))/(n-1)))

            0.42 - (0.50 * cos (((1*pi*(n-1))/(n-1)))
                 + (0.08 * cos (((2*pi*(n-1))/(n-1)))

            0.42 - (0.50 * cos (pi)) + (0.08 * cos (2*pi))

            0.42 - (0.50 * -1) + (0.08 * 1) = 0.42 + 0.5 + 0.08 = 1
    ...
    Example:
        ...
        fract32           blackman_window[N_FFT];

        int block_exponent;


        gen_blackman_fr32 (blackman_window, 1, N_FFT);

        vecvmlt_fr32 (signal, blackman_window,
...
extern void
gen_blackman_fr32 (fract32  _blackman_window[],
                   int      _window_stride,
                   int      _window_size)
{
    /* local copies of the arguments */

    fract32 *__restrict  blackman_window = _blackman_window;
    ...
    if expected_true ((window_size > 1) && (window_stride > 0))
    {
        ...
        */
        blackman_window[n_half*window_stride] = 0x7FFFFFFF;

        /* set first and last value in table to default */
        offset_lowhalf = 0;
        offset_highhalf = (window_size - 1) * window_stride; 
        blackman_window[offset_lowhalf] = 0x0;
        blackman_window[offset_highhalf] = 0x0;
...
#pragma vector_for
        for (i = 1; i < n_half; i++)
        {
            ...
            /* 0.42 - (0.5 * cos ((2*pi*i)/(n-1))) */
            tmp = shr_fr1x32 (__cos32_2PIx (alpha), 1);
            win_value = sub_fr1x32 (0x35c28f5c, tmp);

            /* + (0.08 * cos ((4*pi*i)/(n-1))) */ 
            alpha += alpha;
            tmp = multr_fr1x32x32 (0x0a3d70a4, __cos32_2PIx (alpha));
            win_value = add_fr1x32 (win_value, tmp);

            offset_lowhalf += window_stride;
            offset_highhalf -= window_stride;

            blackman_window[offset_lowhalf] = win_value;
            blackman_window[offset_highhalf] = win_value;
        }
    }
