...
******************************************************************************
  ...
  Label name     : __vecmin_fr16

  Description    : This function returns the smallest array value
...
.file_attr libFunc       = vecmin_fr16;
.file_attr libFunc       = __vecmin_fr16;
.file_attr libName = libdsp;
.file_attr prefersMem    = internal;
.file_attr prefersMemNum = "30";
.file_attr FuncName      = __vecmin_fr16;
#endif
...
__vecmin_fr16:
    ...
    defined(__WORKAROUND_BF532_ANOMALY_050000245)
       ...
       LSETUP( ST_VMIN, ST_VMIN ) LC0 = P1 >> 1; 
ST_VMIN:  R0 = MIN( R2, R0 )(V) || R2 = [P0++]; 
       ...
       CC = BITTST(R1,0);    
       IF !CC R2 = R0;      
       R0 = MIN( R2, R0 )(V);// If number of samples odd, compare
                             // result obtained in loop with last value read
 
       R1 = R0 >>> 16;       // Copy minimum value m1 to 32-bit DREG
       R2 = R0.L (X);        // Sign extend minimum value m2 to 32-bit 
       CC = R2 < R1;
