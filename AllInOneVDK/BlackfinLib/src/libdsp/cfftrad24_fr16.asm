...
******************************************************************************
  ...
  Description    : This file contains the code for a radix-2 implementation
			      ...
			      complex_fract16       *out,  // Output array
                   ...
                   The input, output and temporary arrays are expected to be
...
#ifdef __RADIX_4_FFT
.file_attr libFunc       = __cfftrad4_fr16;
.file_attr libFunc       = cfftrad4_fr16;
.file_attr FuncName      = __cfftrad4_fr16;
#else /* Radix-2 FFT */
.file_attr libFunc       = __cfftN_fr16;
.file_attr libFunc       = cfftN_fr16;
.file_attr FuncName      = __cfftN_fr16;
#endif
#endif
...
/** Function Prologue **/
        ...
        R3 = [SP+44];                  // The value of twiddle stride
        R2 = [SP+48];                  // The value of length of input array

        R4 = 8;
        CC = R2 < R4;                  // Exit if the number of input 
                                       // samples is < 8
...
/* Stage 1 and 2 butterfly */

/* The input array is copied into the temporary buffer in bit-reversed order.
 * Then the stage 1 and 2 of the butterfly structure are implemented. 
 ...
 * scaling purpose. In on e loop data corresponding to 2 butterflies are 
 * processed.
 */
        ...
        M0 = R3;                       // M0 stores the offset required 
                                       // for bit-reversing
        M1 = B1;                       // M1 stores the address for the
                                       // temporary buffer.
        I1 = 0;                        // I1 is initialized to 0 for 
                                       // bit-reversing
        R0 = [I0++];
...
copy_end: 
        ...
        P5 = P4 >> 2;                  // P5 is equal to number of butterflies
                                       // at Stage 2
...
*/
        ...
        //The computation of number of stages (=m) is done here
Find_m:            
        R2 >>= 1;
        R0 += 1;
        CC = R2 == R3;
 ...
 * half of the number of butterflies at each stage. The second nested loop is
 * set for the number of lines in each butterfly. The computation is done on 
 * the output array. The output is stored after dividing by 2 for scaling 
 * purpose. In one loop two butterfly data are read and processed.
 */
...
Loopfor_m:
        ...
        P2 = P0 << 1;
        P1 = P2 + P0;
        M0 = P1;                       // The offset used for third butterfly
        P5 = P5 >> 1;
        R7 = R7 >>> 1 || I1 += M0;
...
Loop1_strt:
           ...
           //Loop for number of lines per butterfly
...
/* Last stage of butterfly */

/* This part implements the last stage of the butterfly. The label Esc_mid
 * is used when the size of input data is 8. In this case the computation
 * of middle stages have to be escaped. The increment in the twiddle factor 
 * offset is just 1. In the last stage there is only one butterfly. 
 * The loop is set for n/4. 4 data are read and processed at the same time.
 */
...
Last_strt:      
           ...
           A1 = R3.L * R6.H, A0 = R3.L * R6.L || [I0++M0] = R4 || R5 = [I1];
#ifdef __RADIX_4_FFT
           R6.H = (A1 -= R3.H * R6.L), R6.L = (A0 += R3.H * R6.H) ||
                                                              R3 = [I3++M2];
#else  /* Radix-2 FFT */
           R6.H = (A1 += R3.H * R6.L), R6.L = (A0 -= R3.H * R6.H) || 
                                                              R3 = [I3++M2];
#endif
           R5 = R5 +|+ R6, R6 = R5 -|- R6 || [I2++M0] = R2 || R4 = [I0];
